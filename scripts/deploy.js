...
for (const key of keysToPush) {
    if (secrets[key]) {
        ...
        runCommand(`npx wrangler pages secret put ${key} --project-name jah`, {}, secrets[key]);
    }
}

// 4. Deploy Final
console.log("\n4. Realizando Deploy para o Cloudflare Pages...");
const outputDir = fs.existsSync(path.resolve(__dirname, '../.output/public')) ? '.output/public' : 'dist';
