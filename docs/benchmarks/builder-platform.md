...
A plataforma conceitual para Waesy deve permitir não apenas a estruturação de páginas através de blocos estáticos, mas também a construção de experiências interativas, tipadas e orientadas a dados (Data Bindings reais). As ferramentas líderes no mercado (Wix Studio, Shopify Hydrogen, Plasmic e Builder.io) permitem criar instâncias baseadas em um motor único.

Para a Waesy, isto significa unificar:
...
- Na Waesy, o Inspector deverá abstrair propriedades CSS complexas em seletores seguros (ex: Spacing, Alignment, Flexbox/Grid simplificado).
...
- Na Waesy, as publicações devem ser versionadas em tabelas relacionais (`experience_versions`), mantendo a integridade com os dados comerciais via Supabase.
