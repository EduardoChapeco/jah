...
- **Checkout Sem Atritos:** Os Bio Links das afiliadas e vendedoras Waesy terão o módulo Mini-Cart e o Botão "Buy Now" embutidos nativamente. O fluxo de compra (mesmo via link da bio) poderá acoplar o PIX sem sair do domínio.
- **Domínio Único e Sessão Compartilhada:** O Bio Link operará sob o ecossistema TanStack Router da Waesy. Isso significa que o visitante que colocar o produto da vitrine da afiliada no carrinho permanecerá "logado" na loja, sem as famosas quebras de sessão (ex.: clicar no Linktree e perder login e carrinho do Safari no WebView do Instagram).
- **Atribuição Indestrutível:** A atribuição da venda (`seller_id`) acontecerá server-side ao processar o evento de visualização do Biolink no backend da Waesy, sobrevivendo a limpezas de cache de front-end.
