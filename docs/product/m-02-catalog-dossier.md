...
**Causa raiz:** O `head()` da rota `_store.produto.$slug.tsx` retorna um título fixo `"Waesy — Produto"` em vez de usar o nome real do produto. Sem `og:image`, `og:description`, ou `canonical`.
