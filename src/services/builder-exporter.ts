...
export function exportStaticHtml(
  ...
  const title = options.title || 'Experiência Interativa JAH';
  const storeName = options.storeName || 'Loja Certificada JAH';
  const desc = options.description || 'Página de alta conversão gerada com JAH Universal Experience Builder';
...
<head>
  ...
  <meta name="generator" content="JAH Universal Experience Builder">
  <!-- Fonts -->
...
<body class="bg-slate-950 text-slate-50 min-h-screen antialiased selection:bg-brand selection:text-white">
  <div id="jah-root" class="w-full flex flex-col items-center">
    ...
    <header class="w-full max-w-6xl py-6 px-4 flex items-center justify-between border-b border-slate-800/80">
      ...
      <div class="text-xs text-slate-400 font-mono">Certificado JAH 360</div>
    </header>
    ...
    <footer class="w-full max-w-6xl py-8 px-4 mt-auto border-t border-slate-800/60 text-center text-xs text-slate-500">
      <p>© ${new Date().getFullYear()} ${storeName}. Desenvolvido com tecnologia JAH Universal Experience OS.</p>
    </footer>
  </div>

  <script>
    window.__JAH_EXPERIENCE_NODES__ = ${serializedNodes};
    console.log("JAH Standalone Experience Loaded", window.__JAH_EXPERIENCE_NODES__.length, "nodes");
  </script>
</body>
</html>`;
}
