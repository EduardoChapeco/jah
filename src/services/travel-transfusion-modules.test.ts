...
export type TravelSupplierKind =
  | 'operadora'
  | 'dmc_receptivo'
  | 'companhia_aerea'
  | 'rede_hoteleira'
...
export interface TravelVisaRecord {
  id: string;
  status: TravelVisaStatus;
}

export type TravelVoucherType =
  | 'pacote_completo'
  | 'hotel'
  | 'aereo'
  | 'passeio'
...
export interface TravelVoucherRecord {
  id: string;
  voucher_code: string;
  store_id: string;
...
export interface TravelDepartureCard {
  id: string;
