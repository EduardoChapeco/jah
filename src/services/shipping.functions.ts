...
/**
 * Calculates shipping via external APIs (Mocked dynamically for development)
 * Stores the generated quotes in the database for auditing and anti-fraud validation on checkout.
 */
export const calculateShipping = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: { cartId, zipcode, weightGrams } }) => {
    ...
    // 1. Fetch Manual Shipping Options (Fallback/Local rules)
    const { data: manualOptions } = await supabase
    ...
    if (manualOptions && manualOptions.length > 0) {
       manualOptions.forEach(opt => {
          finalQuotes.push({
             ...
             estimated_days: opt.estimated_days || 1,
          });
       });
    }
    ...
    if (creds && creds.token_payload?.api_token) {
        try {
            ...
            const response = await fetch('https://sandbox.melhorenvio.com.br/api/v2/me/shipment/calculate', {
                ...
                body: JSON.stringify({
                    from: { postal_code: "01001000" }, // Ex: store origin (should be read from store settings in a full implementation)
            ...
            if (response.ok) {
                ...
                // Parse MelhorEnvio results (Correios PAC, SEDEX, etc)
            ...
            // Graceful degradation: If external API fails, we don't crash, we just rely on manual options.
