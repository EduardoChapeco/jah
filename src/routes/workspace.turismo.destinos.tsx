...
export const Route = createFileRoute("/workspace/turismo/destinos")({
  head: () => ({ meta: [{ title: "Banco de Destinos Turísticos & CMS | Workspace JAH Master OS" }] }),
  loader: async () => {
...
export default function WorkspaceDestinationsPage() {
  ...
  // Busca rápida na base canônica completa
  ...
  return (
    ...
    >
      <div className="space-y-6 pb-20 animate-in fade-in duration-200">
        ...
        <PageHeader
          eyebrow="Turismo & Catálogo de Experiências"
          title="Banco de Destinos Turísticos (CMS Studio)"
          actions={
            <Button
              type="button"
              size="sm"
            ...
            >
              ...
              <span>Novo Destino (Studio CMS)</span>
            </Button>
          }
        />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Card className="p-4 rounded-2xl border border-border/60 bg-card space-y-1 shadow-2xs">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">
              Destinos no Catálogo
            </span>
            <span className="text-2xl font-black text-foreground">{metrics.total}</span>
            <p className="text-[11px] text-muted-foreground">Cadastros padronizados</p>
          </Card>

          <Card className="p-4 rounded-2xl border border-border/60 bg-card space-y-1 shadow-2xs">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">
              Aeroportos IATA Oficiais
            </span>
            <span className="text-2xl font-black text-foreground">{metrics.withIata}</span>
            <p className="text-[11px] text-muted-foreground">Hubs & Gateways mapeados</p>
          </Card>
          ...
          <Card className="p-4 rounded-2xl border border-border/60 bg-card space-y-1 shadow-2xs">
            ...
            <span className="text-2xl font-black text-foreground">{metrics.statesCount}</span>
            <p className="text-[11px] text-muted-foreground">UFs cadastradas</p>
          </Card>
          ...
          <Card className="p-4 rounded-2xl border border-border/60 bg-card space-y-1 shadow-2xs">
            ...
            <span className="text-2xl font-black text-foreground">{metrics.totalHotels}</span>
            <p className="text-[11px] text-muted-foreground">Parceiros hospedagem</p>
          </Card>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar scrollbar-none">
            {[
              ...
              { id: "brasil", label: "Nacionais (Brasil)" },
              { id: "internacional", label: "Internacionais" },
              { id: "iata", label: "Com Aeroporto IATA" },
              { id: "secoes", label: "Com Seções CMS Ricas" },
              { id: "fotos", label: "Com Galeria de Fotos" },
            ].map((f) => (
              <Button
                key={f.id}
                type="button"
                size="sm"
                variant={filterTag === f.id ? "default" : "outline"}
                onClick={() => setFilterTag(f.id)}
                className="rounded-xl text-xs font-bold h-8 px-3 cursor-pointer shrink-0"
              >
                {f.label}
              </Button>
            ))}
          ...
          <div className="relative w-full sm:w-80">
            ...
            <Input
              placeholder="Buscar por cidade, UF, aeroporto IATA (ex: MCZ, Gramado)..."
              value={search}
        ...
        <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
          ...
          >
            ...
            >
              <div className="px-5 pt-3 pb-2 border-b border-border/60 bg-card overflow-x-auto no-scrollbar scrollbar-none">
                <TabsList className="grid grid-cols-6 w-full min-w-[620px] h-9 rounded-xl p-1 bg-muted/50">
