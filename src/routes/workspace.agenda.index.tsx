...
import { Calendar as CalendarIcon, Clock, User, CheckCircle2, XCircle, FileText, Play, Activity } from "lucide-react";
    ...
    <Sheet open={isOpen} onOpenChange={(val) => !val && onClose()}>
      <SheetContent className="sm:max-w-md flex flex-col h-full">
        <SheetHeader>
          ...
          <SheetDescription>
            Histórico clínico de {guestName}
          </SheetDescription>
        </SheetHeader>
        <div className="flex-1 overflow-y-auto py-4 space-y-4">
          {isLoading ? (
            ...
            <p className="text-sm text-muted-foreground italic">Nenhum registro encontrado para este paciente.</p>
          ) : (
            records.map((r: any) => (
              <div key={r.id} className="bg-muted p-3 rounded-md text-sm border border-border">
...
function AdminAppointmentsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "pending":
      case "confirmed":
        return <Badge variant="outline" className="bg-amber-500/10 text-amber-600 border-amber-500/20">Aguardando</Badge>;
      case "checked_in":
        return <Badge variant="outline" className="bg-indigo-500/10 text-indigo-600 border-indigo-500/20">Aguardando na Recepção</Badge>;
      case "in_service":
        return <Badge variant="outline" className="bg-violet-500/10 text-violet-600 border-violet-500/20">Em Atendimento</Badge>;
      case "completed":
        return <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20">Concluído</Badge>;
      case "no_show":
        return <Badge variant="outline" className="bg-zinc-500/10 text-zinc-600 border-zinc-500/20">Não Compareceu</Badge>;
      case "cancelled":
        return <Badge variant="outline" className="bg-rose-500/10 text-rose-600 border-rose-500/20">Cancelado</Badge>;
      default:
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      <PageHeader
        eyebrow="CRM Clínico & Agendamentos"
        title="Agenda Orquestrada"
        description="Controle o ciclo de vida: Check-in, Atendimento, Prontuários e No-Shows."
      />
        ...
        <div className="flex-1 overflow-x-auto pb-4">
          <div className="flex gap-6 min-w-max h-full">
            {Object.keys(groupedAppointments).map((resourceName) => (
              <div
                key={resourceName}
                className="w-96 flex flex-col gap-4 bg-muted/20 p-4 rounded-xl border border-border/50"
              >
                ...
                <div className="flex flex-col gap-3 flex-1 overflow-y-auto no-scrollbar">
                  {groupedAppointments[resourceName].map((appt: any) => (
                    <div
                      key={appt.id}
                      className="flex flex-col border border-border/80 bg-card rounded-md p-3 overflow-hidden"
                    >
                      ...
                      <div className="pt-2 border-t border-border mt-2">
                        <p className="text-sm font-semibold text-foreground/90 flex items-center justify-between">
                          ...
                          <Button 
                            ...
                            onClick={() => setSelectedAppt({ id: appt.id, name: appt.guest_name || "Cliente" })}
                          >
                      ...
                      {['pending', 'confirmed'].includes(appt.status) && (
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          <Button size="xs" variant="outline" onClick={() => statusMutation.mutate({ id: appt.id, status: 'checked_in' })}>
                            Fazer Check-in
                          </Button>
                          <Button size="xs" variant="ghost" className="text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => statusMutation.mutate({ id: appt.id, status: 'no_show' })}>
                            Faltou
                          </Button>
                        </div>
                      )}
                      {appt.status === 'checked_in' && (
                        <div className="mt-3 flex gap-2">
                          <Button size="xs" className="w-full" onClick={() => statusMutation.mutate({ id: appt.id, status: 'in_service' })}>
                      ...
                      {appt.status === 'in_service' && (
                        <div className="mt-3 flex gap-2">
                          <Button size="xs" variant="success" className="w-full" onClick={() => statusMutation.mutate({ id: appt.id, status: 'completed' })}>
