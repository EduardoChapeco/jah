...
function AbandonedCartsPage() {
  ...
  const handleContactWhatsApp = async (cart: any) => {
    ...
    try {
      ...
      window.open(
        `https://wa.me/55${cart.customerPhone.replace(/\D/g, "")}?text=${encodeURIComponent(message)}`,
        "_blank",
      );
    } catch (e: any) {
      ...
      {carts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {carts.map((cart: any) => {
            ...
            const total = cart.snapshot.items.reduce(
              (acc: number, item: any) => acc + item.price_cents * item.qty,
              0,
            );
            ...
            return (
              <div
                key={cart.id}
                className="bg-card border overflow-hidden flex flex-col justify-between"
              >
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Há{" "}
                        {Math.floor(
                          (Date.now() - new Date(cart.createdAt).getTime()) / (1000 * 60 * 60),
                        )}{" "}
                        horas
                      </p>
                    </div>
                ...
                <div className="p-4 flex-1">
                  ...
                  {cart.recoveryAttempts > 0 && (
                    <div className="inline-flex items-center text-xs bg-warning text-warning px-2 py-1 rounded-md mb-2">
                ...
                <div className="p-4 border-t bg-card grid grid-cols-2 gap-2">
                  <Button
                    ...
                    onClick={() => handleContactWhatsApp(cart)}
                  >
