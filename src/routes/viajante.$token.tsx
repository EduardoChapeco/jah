...
import { CheckCircle2, Globe, User, Phone, ShieldCheck, HeartPulse, Send, AlertCircle } from 'lucide-react';
...
export const Route = createFileRoute('/viajante/$token')({
 head: () => ({ meta: [{ title: 'Ficha Cadastral do Passageiro | TravelOS' }] }),
...
function PublicTravelerFormPage() {
 ...
 setLoading(true);
 try {
 // Direct call to traveler_forms via Supabase or local state
 setSubmitted(true);
 toast.success('Ficha cadastral enviada com sucesso para a agência!');
 } catch (err: any) {
 toast.error('Erro ao enviar ficha: ' + err?.message);
 } finally {
 setLoading(false);
 }
 };
 ...
 <h2 className="text-2xl font-black text-white">Ficha Confirmada!</h2>
 <p className="text-xs text-slate-400 max-w-sm mx-auto leading-relaxed">
 Obrigado, <strong className="text-white">{fullName}</strong>! Seus dados e documentos foram recebidos com segurança pela agência para emissão de passagens e seguro viagem.
 </p>
 <div className="pt-2">
 ...
 Ficha Segura do Viajante
 </span>
 <h1 className="text-xl font-black text-white mt-0.5">Identificação & Documentação</h1>
 </div>
 ...
 {loading ? 'Enviando...' : 'Finalizar & Enviar Ficha'}
 </Button>
 )}
 </div>
 </form>
