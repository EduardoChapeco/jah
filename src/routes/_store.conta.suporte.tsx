...
function CustomerSupportPage() {
  ...
  return (
    <div className="space-y-6 flex flex-col h-[70vh]">
      ...
      <div className="flex-1 bg-card border rounded-lg overflow-hidden flex flex-col md:flex-row">
        
        {/* Ticket List - Responsive hide when a ticket is selected on mobile */}
        <div className={`w-full md:w-80 border-r bg-muted/10 flex flex-col ${selectedTicketId ? 'hidden md:flex' : 'flex'}`}>
                  ...
                  {tickets.map((t: any) => (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center mt-2">
                        ...
                        {t.status === "open" && <Badge variant="secondary" className="text-[10px]">Aguardando Loja</Badge>}
                        {t.status === "waiting_customer" && <Badge variant="destructive" className="text-[10px]">Sua Vez</Badge>}
        ...
        {/* Chat Area */}
        <div className={`flex-1 flex flex-col bg-background/50 relative ${!selectedTicketId ? 'hidden md:flex' : 'flex'}`}>
          {!selectedTicketId ? (
            ...
            <>
              ...
              <div className="p-4 border-b bg-card flex justify-start items-center gap-3">
                 <Button variant="ghost" size="icon" className="md:hidden" onClick={() => setSelectedTicketId(null)}>
                   <ChevronLeft className="w-5 h-5" />
                 </Button>
                 {loadingThread ? (
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {thread?.messages.filter((m: any) => !m.isInternal).map((m: any) => (
                  <div key={m.id} className={`flex ${m.isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] md:max-w-[70%] rounded-lg p-3 ${
                      ...
                      <div className="flex items-center gap-2 mb-1 opacity-70">
                        ...
                        <span className="text-[10px] font-medium">{m.isMe ? "Você" : "Equipe da Loja"}</span>
