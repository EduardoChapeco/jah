...
export const Route = createFileRoute("/workspace/turismo/hoteis")({
  head: () => ({ meta: [{ title: "Banco de Hotéis & Resorts | Workspace JAH Master OS" }] }),
  loader: async () => {
  ...
  return (
    ...
    >
      <div className="space-y-6 pb-20">
        <PageHeader
          eyebrow="Turismo & Hotelaria Enterprise"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="p-4 rounded-2xl bg-card border border-border/60 flex items-center gap-3 shadow-2xs">
            <div className="size-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center shrink-0">
              <Hotel className="size-5" />
            </div>
            <div>
              <p className="text-[11px] font-medium text-muted-foreground">Hospedagens no Banco</p>
              <p className="text-xl font-bold text-foreground">{totalHotels}</p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-card border border-border/60 flex items-center gap-3 shadow-2xs">
            ...
            <div>
              <p className="text-[11px] font-medium text-muted-foreground">Resorts All Inclusive</p>
              <p className="text-xl font-bold text-foreground">{allInclusiveCount}</p>
            </div>
          </div>

          <div className="p-4 rounded-2xl bg-card border border-border/60 flex items-center gap-3 shadow-2xs">
            <div className="size-10 rounded-xl bg-amber-400/10 text-amber-500 flex items-center justify-center shrink-0">
              <Award className="size-5" />
            </div>
            <div>
              <p className="text-[11px] font-medium text-muted-foreground">5 Estrelas (Luxo)</p>
              <p className="text-xl font-bold text-foreground">{fiveStarsCount}</p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-card border border-border/60 flex items-center gap-3 shadow-2xs">
            ...
            <div>
              <p className="text-[11px] font-medium text-muted-foreground">Satisfação Média</p>
              <p className="text-xl font-bold text-foreground">{averageRating} / 5.0</p>
            </div>
          </div>
        </div>

        {/* ── BARRA DE CONTROLE, FILTROS & VISUALIZAÇÃO ── */}
        <div className="p-3 rounded-2xl bg-card border border-border/70 flex flex-col md:flex-row items-center justify-between gap-3 shadow-2xs">
          <div className="flex-1 w-full flex flex-col sm:flex-row items-center gap-2">
            ...
            <div className="relative flex-1 w-full">
              ...
              <Input
                ...
                placeholder="Buscar por hotel, resort, cidade, estado ou comodidade..."
                className="pl-9 h-9 rounded-xl bg-background text-xs border-border/70 w-full"
              />
            </div>
            ...
            {destinations.length > 0 && (
              <Select value={selectedDestination} onValueChange={setSelectedDestination}>
                <SelectTrigger className="h-9 w-full sm:w-44 rounded-xl bg-background text-xs border-border/70">
            ...
            <Select value={selectedRegime} onValueChange={setSelectedRegime}>
              <SelectTrigger className="h-9 w-full sm:w-40 rounded-xl bg-background text-xs border-border/70">
            ...
            <Select value={selectedStars} onValueChange={setSelectedStars}>
              <SelectTrigger className="h-9 w-full sm:w-36 rounded-xl bg-background text-xs border-border/70">
                <SelectValue placeholder="Estrelas" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Todas Estrelas</SelectItem>
          ...
          <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
            {/* Toggle de Visualização Grid vs Tabela */}
            <div className="flex items-center bg-muted/60 p-0.5 rounded-xl border border-border/60">
              ...
              <button
                ...
                title="Modo Tabela Operacional"
              >
                <TableIcon className="size-3.5" />
              </button>
            </div>

            <Badge variant="outline" className="h-9 px-3 rounded-xl font-mono text-xs border-border/70 shrink-0">
                    ...
                    <tr key={hotel.id} className="hover:bg-muted/20 transition-colors">
                      <td className="p-3">
                        <div className="flex items-center gap-3">
                          <img
                            src={hotel.cover_photo_url || "https://images.unsplash.com/photo-1571896349842-33c89424de2d?q=80&w=200"}
                            alt={hotel.name}
        ...
        {/* ── MODAL RAIO-X DO HOTEL & RESORT (TRUTHFUL PREVIEW) ── */}
        <Dialog open={!!previewHotel} onOpenChange={(open) => !open && setPreviewHotel(null)}>
          <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto no-scrollbar p-0 bg-card rounded-2xl">
            {previewHotel && (
              <div className="space-y-4">
                {/* Header com Foto de Capa Panorâmica */}
                <div className="relative aspect-[21/9] w-full bg-muted/40 overflow-hidden">
                  <img
                    src={previewHotel.cover_photo_url || "https://images.unsplash.com/photo-1571896349842-33c89424de2d?q=80&w=1400"}
