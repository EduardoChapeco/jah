...
export const Route = createFileRoute('/claim/reivindicar/$entityId')({
  head: () => ({ meta: [{ title: 'Reivindicar Empresa ou Perfil Oficial | JAH Trust' }] }),
  ...
  if (isSubmitted) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="max-w-md w-full p-8 rounded-2xl border border-border bg-card/80 backdrop-blur-xl shadow-2xl text-center flex flex-col items-center">
          <div className="size-16 rounded-full bg-emerald-500/10 text-emerald-500 flex items-center justify-center mb-4">
            <CheckCircle2 className="size-8" />
          </div>
          <h2 className="text-2xl font-bold tracking-tight text-foreground">Solicitação em Auditoria</h2>
          ...
          <div className="w-full mt-6 p-4 rounded-2xl bg-muted/40 text-left text-xs space-y-1">
            <p className="text-muted-foreground">E-mail de confirmação enviado para:</p>
            <p className="font-semibold text-foreground">{requesterEmail}</p>
          </div>
          <Button 
      ...
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8">
          <Badge variant="outline" className="px-3 py-1 mb-3 rounded-full text-xs font-semibold gap-1.5 border-primary/30 text-primary">
            <ShieldCheck className="size-3.5" /> JAH Trust & Compliance
          </Badge>
        ...
        <form onSubmit={handleSubmit} className="p-6 sm:p-8 rounded-2xl border border-border bg-card shadow-xl space-y-6">
              ...
              {PROOF_OPTIONS.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-2xl border text-left flex items-start gap-3 transition-all min-h-[44px] ${
                      isSelected 
