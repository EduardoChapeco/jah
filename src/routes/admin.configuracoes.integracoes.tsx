...
const INTEGRATION_CATALOG = [
  {
    id: "mercado_pago",
    name: "Mercado Pago",
    description: "Checkout transparente, Pix e Cartão via Mercado Pago.",
    icon: Settings,
    color: "text-blue-500",
    bg: "bg-blue-100 dark:bg-blue-900/30",
    fields: [
      { key: "public_key", label: "Public Key", type: "text" },
      { key: "access_token", label: "Access Token (Secret)", type: "password" }
    ],
  },
  {
    id: "stripe",
    name: "Stripe",
    description: "Pagamento global com alta conversão.",
    icon: Settings,
  ...
  },
  {
    ...
    fields: [{ key: "api_token", label: "Token de Acesso (Bearer)", type: "password" }],
  },
  {
    id: "frenet",
    name: "Frenet",
    ...
    fields: [{ key: "api_token", label: "Chave de Acesso (API Key)", type: "password" }],
  },
  {
    id: "meta_pixel",
  ...
  },
  {
    ...
    fields: [{ key: "measurement_id", label: "ID da Métrica (G-XXXXXXXXXX)", type: "text" }],
  },
];
...
function IntegrationsSettingsPage() {
  ...
  const isFormValid = selectedCatalogItem?.fields.every(
    (f: any) => formData[f.key] && formData[f.key].trim() !== "",
  );
  ...
  const handleDelete = async (provider: string) => {
    if (
      !confirm(
        "Tem certeza que deseja remover esta integração? A loja perderá a comunicação imediata.",
      )
    )
      return;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {INTEGRATION_CATALOG.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="bg-card border p-5 flex flex-col justify-between relative overflow-hidden group hover:border-primary/50 transition-colors"
            >
              <div className="flex justify-between items-start mb-4">
                <div className={`p-3 ${item.bg}`}>
      ...
      <Sheet open={modalOpen} onOpenChange={setModalOpen}>
        <SheetContent>
          <SheetHeader>
            ...
            <SheetDescription>
              Insira as chaves de acesso fornecidas pela plataforma. Elas serão salvas em nosso
              cofre de segurança criptografado.
            </SheetDescription>
          </SheetHeader>
              ...
              <div className="flex items-center justify-between pt-4 border-t mt-4">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    O sistema consumirá os dados desta API.
                  </p>
                </div>
                <Switch
          ...
          <SheetFooter className="flex justify-between items-center sm:justify-between mt-8">
            {activeIntegrations.find((a: any) => a.provider === selectedCatalogItem?.id) ? (
              <Button
                variant="destructive"
                size="icon"
                onClick={() => handleDelete(selectedCatalogItem.id)}
              >
