...
import { Search, MapPin, Loader2, AlertCircle, ChevronDown, MessageSquare } from "lucide-react";
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pb-20">
      ...
      <form
        ...
        className="sticky top-0 z-30 bg-secondary px-4 py-4 border-b border-border shadow-md flex items-center gap-3"
      >
        <div className="flex-1 flex items-center bg-background border border-border shadow-sm px-4 py-3">
          <Search className="size-6 text-foreground mr-3 shrink-0" />
          <input
            type="search"
            value={searchQuery}
            ...
            className="bg-transparent border-none outline-none w-full text-foreground placeholder:text-foreground/50 font-mono text-sm md:text-base font-bold uppercase tracking-wider"
          />
        </div>
        ...
        <Link to="/diretorio">
          <Button
            ...
            className="shrink-0 w-14 h-14 bg-secondary text-foreground hover:bg-secondary/90"
          >
      ...
      <div className="p-4 md:p-8 space-y-10 max-w-2xl mx-auto w-full mt-4">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 border-b border-border pb-6">
          <h1 className="text-display text-5xl md:text-7xl text-foreground leading-none">
            MURAL DA <br />
        ...
        {!isLoading && !isError && allItems.length === 0 && (
          <div className="relative rotate-1 transition-all duration-300">
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-32 h-10 bg-background/80 backdrop-blur-sm border border-border/20 -rotate-3 z-10 shadow-sm" />
            <Surface
              variant="default"
              padding="lg"
            ...
            >
              <div className="absolute -right-6 -top-6 rotate-12">
                <div className="stamp-badge text-2xl px-4 py-2 border shadow-sm">VAZIO</div>
              </div>
              <div className="py-12 flex flex-col items-center justify-center">
                <div className="size-24 rounded-full border border-border border-dashed flex items-center justify-center mb-6">
                  <MessageSquare className="size-10 text-foreground/40" />
                </div>
                <h2 className="font-display text-4xl mb-3 uppercase tracking-tighter">
                  O Silencio das Ruas
                </h2>
                <p className="font-sans text-muted-foreground text-lg text-foreground/80 max-w-md mx-auto mb-8">
                  Nenhuma publicacao encontrada. Seja o primeiro a colar um cartaz, anunciar uma
                  mercadoria ou divulgar um evento.
                </p>
                <Button
                  asChild
