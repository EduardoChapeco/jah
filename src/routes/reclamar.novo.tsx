...
export const Route = createFileRoute('/reclamar/novo')({
  head: () => ({ meta: [{ title: 'Registrar Reclamação do Consumidor | JAH Reclamar & JUS' }] }),
...
function ReclamarNovoPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setIsSuccess(true);
      toast.success('Reclamação registrada com sucesso no JAH Reclamar!');
    } catch (err: any) {
      ...
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <Badge variant="outline" className="px-3 py-1 mb-3 rounded-full text-xs font-semibold gap-1.5 border-rose-500/30 text-rose-500">
            <AlertTriangle className="size-3.5" /> JAH Reclamar & Proteção ao Consumidor
          </Badge>
