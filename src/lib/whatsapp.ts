...
export function buildTrackedWhatsAppMessage({
 ...
 switch (entityType) {
 case "product":
 return `Olá! Vi o produto "${entityTitle || "anunciado"}" no JAH e gostaria de mais informações sobre disponibilidade e entrega.\n\nRef: #${leadCode}`;
 case "classified":
 return `Olá! Vi seu anúncio "${entityTitle || "no JAH"}" e tenho interesse. Ainda está disponível?\n\nRef: #${leadCode}`;
 case "job":
 return `Olá! Vi a oportunidade "${entityTitle || "de emprego"}" no portal JAH e gostaria de me candidatar.\n\nRef: #${leadCode}`;
 case "tourism":
 return `Olá! Vi a atração/pousada "${entityTitle || "no JAH"}" e gostaria de consultar tarifas e reservas.\n\nRef: #${leadCode}`;
 case "directory":
 case "store":
 return `Olá! Encontrei o perfil de vocês no guia JAH e gostaria de fazer um orçamento.\n\nRef: #${leadCode}`;
 default:
 return `Olá! Vi o anúncio "${entityTitle || "no JAH"}" e gostaria de mais detalhes.\n\nRef: #${leadCode}`;
 }
}
