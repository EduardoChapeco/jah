...
BEGIN
  ...
  INSERT INTO public.directory_listings (
    ...
    NEW.name,
    v_category,
    coalesce(NEW.settings->>'description', 'Empresa oficial cadastrada no ecossistema Wider.'),
...
BEGIN
  FOR r IN SELECT * FROM public.stores LOOP
    ...
    INSERT INTO public.directory_listings (
      ...
      r.name,
      v_cat,
      coalesce(r.settings->>'description', 'Empresa oficial cadastrada no ecossistema Wider.'),
