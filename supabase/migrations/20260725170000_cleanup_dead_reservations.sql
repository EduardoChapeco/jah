-- ============================================================================
-- Waesy Commerce — Migration 20260725170000: Cleanup Dead Reservations
-- ============================================================================
...
-- 1. Drop unused tables and functions related to the ghost reservation system
DROP TABLE IF EXISTS public.stock_reservations CASCADE;
DROP FUNCTION IF EXISTS public.reserve_stock_atomic CASCADE;
DROP FUNCTION IF EXISTS public.release_expired_reservations CASCADE;
DROP FUNCTION IF EXISTS public.release_stock_for_cart_item CASCADE;
DROP FUNCTION IF EXISTS public.process_checkout_atomic CASCADE; -- legacy v1/v3

-- 2. Drop the column from product_variants
ALTER TABLE public.product_variants DROP COLUMN IF EXISTS stock_reserved;

-- 3. Recreate add_to_cart_atomic_v4 to remove reference to stock_reserved
...
-- 4. Recreate process_checkout_transaction_v2 to remove stock_reservations references
