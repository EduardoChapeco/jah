-- ============================================================================
-- Waesy Commerce — Migration 0052: Commission Engine (Microfase 4)
-- ============================================================================
-- 1. Adds commission_rate to profiles.
-- 2. Modifies POS and Checkout RPCs to automatically calculate and record
--    commissions into the `commissions` table atomically.

BEGIN;

-- 1. Add commission_rate to profiles (Default 5%)
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS commission_rate DECIMAL(5,2) DEFAULT 5.00;

COMMIT;

-- 2. Re-create process_pos_sale_transaction to include commission insertion
...
BEGIN
  ...
  -- Calculate Totals and Build Snapshot
  FOR v_item IN (SELECT * FROM jsonb_array_elements(p_items)) LOOP
    ...
    v_items_snapshot := v_items_snapshot || jsonb_build_object(
      'variant_id', v_item->>'variantId', 'qty', (v_item->>'qty')::INTEGER,
      'unit_price_cents', (v_item->>'priceCents')::INTEGER, 'total_cents', ((v_item->>'qty')::INTEGER * (v_item->>'priceCents')::INTEGER),
      'product_title', v_item->>'title', 'variant_sku', v_item->>'sku'
    );
  END LOOP;
  ...
  FOR v_item IN (SELECT * FROM jsonb_array_elements(v_items_snapshot)) LOOP
    INSERT INTO public.order_items (
      ...
      v_order_id, (v_item->>'variant_id')::UUID, v_item->>'product_title', v_item->>'variant_sku', '{}'::JSONB, NULL, 
    ...
    INSERT INTO public.stock_movements (
      ...
      (v_item->>'variant_id')::UUID, p_store_id, 'sale', -1 * (v_item->>'qty')::INTEGER, 'order', v_order_id, 'PDV #' || v_order_public_token, p_seller_id
    );
  ...
  -- COMMISSION ENGINE: Calculate and insert commission
...
-- 3. Re-create process_checkout_transaction_v2 to accept p_affiliate_id and record commissions
...
BEGIN
  ...
  FROM public.orders WHERE public_token = v_order_token;
  ...
  -- 5. COMMISSION ENGINE: Calculate and insert commission for affiliate
