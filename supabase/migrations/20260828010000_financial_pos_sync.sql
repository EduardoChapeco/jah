-- Migration: 20260828010000_financial_pos_sync.sql
-- Description: Injects explicit financial_transactions generation for POS sales into process_pos_sale_transaction.
-- Fixes the massive gap where physical POS sales were completely omitted from the DRE (Income Statement).
...
BEGIN
  ...
  -- 7.5 Lançamento no DRE (Financial Ledger) -> NEW FIX!
  -- Without this, POS Sales were totally invisible to the corporate income statement.
  INSERT INTO public.financial_transactions (
    ...
    v_total_cents,
    v_order_id,
    'Venda PDV Balcão #' || COALESCE(v_order_public_token, v_order_id::TEXT),
    'Vendas Físicas (PDV)',
    CURRENT_DATE
  );
