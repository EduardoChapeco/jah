-- Checkout Backorder Support
-- Refactors process_checkout_transaction_v2 to allow checkout if allow_backorder is true.
...
CREATE OR REPLACE FUNCTION public.process_checkout_transaction_v2(
  ...
  p_manual_payment_method_id UUID DEFAULT NULL,
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Carrinho n√£o encontrado ou j√° processado.';
  END IF;
  ...
  FOR v_item IN (
    ...
    IF v_item.qty > v_item.stock_on_hand AND NOT COALESCE(v_item.allow_backorder, false) THEN
      RAISE EXCEPTION 'O item (SKU: %) est√° esgotado ou tem quantidade insuficiente.', v_item.sku;
    END IF;
  ...
  IF v_cart.coupon_code IS NOT NULL THEN
    ...
    IF NOT FOUND THEN RAISE EXCEPTION 'Cupom inv√°lido ou inativo.'; END IF;
    ...
    IF v_coupon.min_purchase_cents IS NOT NULL AND v_subtotal_cents < v_coupon.min_purchase_cents THEN RAISE EXCEPTION 'Subtotal inferior ao m√≠nimo exigido pelo cupom.'; END IF;
...
-- Fixes add_to_cart_atomic_v4 to ensure it respects variant status and backorder.
...
BEGIN
  ...
  SELECT pv.stock_on_hand, pv.allow_backorder INTO v_available, v_allow_backorder
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Variante n„o encontrada, inativa ou n„o pertence a esta loja.';
  END IF;
  ...
  IF (v_existing_qty + p_qty) > v_available AND NOT COALESCE(v_allow_backorder, false) THEN
    RAISE EXCEPTION 'Estoque insuficiente para esta variaÁ„o.';
  END IF;
