-- ============================================================================
-- Waesy Commerce — Migration 0054: Match Time Checkout Engine (Microfase 5)
-- ============================================================================
    ...
    IF FOUND AND v_offer.status = 'active' THEN
      IF v_offer.expires_at > now() THEN
        ...
        IF EXISTS (SELECT 1 FROM public.order_items WHERE order_id = v_order_id AND variant_id IN (SELECT id FROM public.product_variants WHERE product_id = v_offer.product_id)) THEN
           ...
           -- NOTE: Realistically we should apply the % to the specific item price, but for simplicity we apply it proportionally or globally if it's a single item cart.
           -- Assuming the Match Time discount applies to the cart's subtotal of that item. We simplify by just taking the % off the whole order if it matches.
