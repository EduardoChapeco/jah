...
-- DIRETIVA BIGTECH: Módulo de Carnê Digital & Gestão Completa de Parcelas
...
-- 1. Ampliação do ENUM/Check em personal_financial_entries para suportar 'installment'
...
-- 2. Evolução da Tabela receivables (Cabeçalho do Carnê / Financiamento)
...
-- 3. Evolução da Tabela receivable_installments (Parcelas do Carnê)
...
-- 4. Tabela de Auditoria Imutável de Renegociações & Ajustes de Parcelas
...
-- 5. Habilitação de RLS e Políticas de Segurança
...
-- 6. Stored Procedure: approve_installment_conciliation (Transação Atômica com Espelhamento)
...
BEGIN
  ...
  -- 2. Obter o recebível/carnê
  SELECT * INTO v_rec FROM public.receivables WHERE id = v_inst.receivable_id FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Carnê não encontrado para a parcela: %', p_installment_id;
  END IF;
  ...
  -- 5. Espelhamento atômico no Financeiro Pessoal do devedor
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public.personal_financial_entries (
      ...
      true,
      v_token,
      'Quitação de carnê aprovada pela loja. Lançamento imutável de telemetria contábil.'
...
-- 7. Stored Procedure: create_receivable_with_installments (Emissão Atômica de Carnê)
...
-- 8. Stored Procedure: recalculate_installment_interest (Cálculo de Atraso e Juros)
