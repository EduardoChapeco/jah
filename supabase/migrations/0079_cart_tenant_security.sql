-- Migration 0079: Cart Tenant Security
-- Fixes critical multi-tenant vulnerability where a customer in Store A 
-- could add a product from Store B to their cart.
...
BEGIN
  ...
  -- 2. Validate Stock Availability AND Tenant Isolation
  SELECT (pv.stock_on_hand - pv.stock_reserved) INTO v_available
