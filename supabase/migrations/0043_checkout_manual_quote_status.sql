-- ============================================================================
-- Waesy Commerce — Migration 0043: Checkout Manual Quote Status Update
-- ============================================================================
...
DECLARE
  ...
  v_status public.order_status := 'awaiting_payment';
BEGIN
  -- 1. Idempotency Check
  ...
  -- 2. Lock the Cart AND Check Ownership (Anti-Hijacking)
  IF p_customer_id IS NOT NULL THEN
    SELECT * INTO v_cart
    FROM public.carts
    WHERE id = p_cart_id AND status = 'active' AND customer_id = p_customer_id
    FOR UPDATE;
  ELSE
    SELECT * INTO v_cart
    FROM public.carts
    WHERE id = p_cart_id AND status = 'active' AND session_token = p_session_token
    FOR UPDATE;
  END IF;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Carrinho não encontrado, já processado, ou acesso negado.';
  END IF;
  ...
  FOR v_item IN (
    ...
    -- Anti-Negative Stock Check
    IF v_item.stock_on_hand < v_item.qty THEN
      RAISE EXCEPTION 'Estoque insuficiente para o produto % (SKU: %). Solicitado: %, Disponível: %', 
        v_item.product_title, COALESCE(v_item.sku, 'N/A'), v_item.qty, v_item.stock_on_hand;
    END IF;
  ...
  -- Determine final order status based on shipping method
  IF p_shipping_method = 'manual_quote' THEN
    v_status := 'awaiting_shipping_quote';
    -- For quote, shipping cents starts at 0 until calculated by staff
    v_shipping_cents := 0;
  END IF;
  ...
  INSERT INTO public.orders (
    ...
    v_store_id, v_cart.customer_id, v_status, v_items_snapshot,
